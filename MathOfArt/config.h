...
struct Config {
	...
	double coordScaleX=10.0, coordScaleY = 10.0;
	
	//LANDSCAPE
	double displacement = 0.3;
	double roughness = 0.5;
	bool is2d = true;
	double lsScale = 100.0;
	...
	void readConfig(std::string path) {
		try {
			...
			seed = readParam<int>(config, "main.seed", seed);
			...
			else if (type == Config::Type::BROWNIAN) {
				drawCircles = readParam<int>(config, "brownian.circle_radius", drawCircles);
				iterations = readParam<int>(config, "brownian.max_iterations", iterations);
				lineWidth = readParam<int>(config, "brownian.line_width", lineWidth);
				distance = readParam<int>(config, "brownian.distance", distance);
			}
			else if (type == Config::Type::DLA) {
				radius = readParam<int>(config, "dla.radius", radius);
				iterations = readParam<int>(config, "dla.max_iterations", iterations);
			}
			else if (type == Config::Type::ATTRACTOR) {
				iterations = readParam<int>(config, "attractor.max_iterations", iterations);
				scale = readParam<int>(config, "attractor.scale", scale);
				colorScale = readParam<double>(config, "attractor.color_scale", colorScale);
				script = readParam<std::string>(config, "attractor.script", script);
			}
			else if (type == Config::Type::FERN) {
				iterations = readParam<int>(config, "fern.max_iterations", iterations);
				coordScaleX = readParam<double>(config, "fern.scale_x", coordScaleX);
				coordScaleY = readParam<double>(config, "fern.scale_y", coordScaleY);

				auto table = config->get_table("fern");


				auto params = table->get_array_of<cpptoml::array>("params");
				if (params) {
					for (auto v : *params) {
						auto cs = v->get_array_of<double>();
						if (!cs) {
					...
					I_LOG("Can't find key [fern]: params");
				}
			}
			else if (type == Config::Type::LANDSCAPE) {
				displacement = readParam<double>(config, "landscale.displacement", displacement);
				roughness = readParam<double>(config, "landscale.roughness", roughness);
				lsScale = readParam<double>(config, "landscale.scale_factor", lsScale);
				is2d = readParam<bool>(config, "landscale.is2d", is2d);
			}
			else if (type == Config::Type::LIGHTNING) {
				penSize = readParam<int>(config, "lightning.pen_size", penSize);
				spred = readParam<int>(config, "lightning.spred", spred);
			}
